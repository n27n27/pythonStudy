...
balls.append({
    ...
    "pos_y": 50,
    "image_idx": 0,
    "to_x": 3, # x 축 이동 방향(+ 면 오른쪽, -면 왼쪽)
    "to_y": -6, # y축 이동 방향(+ 면 아래로, -면 위로)
...
while running:
    dt = clock.tick(30) # 화면의 초당 프레임 수를 30으로 설정
    ...
    # 천장에 닿은 무기 없애기
    weapons = [ [w[0], w[1]] for w in weapons if w[1] > 0 ] # x 좌표가 0보다 큰(천장에 닿지 않은) 값만 저장
